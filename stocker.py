...
class Stocker():
    ...
    def changepoint_prior_analysis(self, title, changepoint_priors=[0.001, 0.05, 0.1, 0.2], colors=['b', 'r', 'grey', 'gold']):
        ...
        # 垂直紅線
        plt.vlines(x=max(self.stock['Date']), ymin=min(future['yhat_lower']), ymax=max(future['yhat_upper']), colors = 'gray',
        linestyles='dashed', label = '現在位置')
        # Actual observations
        ax.plot(train['ds'], train['y'], 'ko', linestyle = "solid", ms = 1,linewidth = 2, label = '實際股價')
        ...
        for prior in changepoint_priors:
            ...
            ax.plot(predictions['ds'], predictions['%.3f_yhat' % prior], linewidth = 2,
                     color = "r", label = '模型預測')
            
            # Plot the uncertainty interval
            ax.fill_between(predictions['ds'].dt.to_pydatetime(), predictions['%.3f_yhat_upper' % prior],
                            predictions['%.3f_yhat_lower' % prior], facecolor = "r",
                            alpha = 0.3, edgecolor = 'k', linewidth = 0.6)
                            
        # Plot labels
        plt.legend(loc = 2, prop={'size': 10})
        plt.xlabel('日期'); plt.ylabel('股價'); plt.title('股價趨勢預測');
        # plt.show()
        path = "todayResult/"+title
